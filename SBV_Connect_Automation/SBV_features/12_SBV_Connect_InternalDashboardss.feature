Feature: Internal Dashboards
  ...
  @WeeklyRegression
  Scenario Outline: Internal Dashboards
    ...
    And Internal User Clicks on Select Filters
    And Internal User Select Region,CashCentre,CaseType,Case_Subtype By using SheetName "<SheetName>" and row number "<rowNumber>" in Internal Connect System
    And Internal User Enters SearchwithRegion value in SearchField and click on ApplyFilters By using SheetName "<SheetName>" and row number "<rowNumber>" in Internal Connect System
    And Internal User Enters CashCenter value in SearchField and click on ApplyFilters By using SheetName "<SheetName>" and row number "<rowNumber>" in Internal Connect System
    And Internal User Enters Case_type value in SearchField and click on ApplyFilters By using SheetName "<SheetName>" and row number "<rowNumber>" in Internal Connect System
    And Internal User Enters Case_Subtype value in SearchField and click on ApplyFilters By using SheetName "<SheetName>" and row number "<rowNumber>" in Internal Connect System
    Then User able to Logout into Connect System
    And User switch to Tab "2"
    Given Internal User able to Login into Connect System with Internal_Username and Internal_password
    And User switch to Tab "3"
