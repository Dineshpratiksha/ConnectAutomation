Feature: Clo Flow_Unhappy_Survey

  @WeeklyRegression
  Scenario Outline: Clo Flow_Unhappy_Survey
    ...
    And User Select "Not satisfied" as the Satisfication Review
    And User Select "Bad" as the rate of level of Service and Enter the Comment Submit the Case
    Then User able to Logout into Connect System
    And User switch to Tab "0"
    Given Reginal User able to Login into Connect system With olga_Username and Olga_Password
    And User switch to Tab "1"
    ...
    And User switch to Tab "0"
    And User enters OTP
    And Reginal User Navigates to TaskList,Search with Latest Case id
    And Reginal User Click on RTL Review
    And Update the status as "Resolved"
    And Reginal User Click on Update Button
    Then User able to Logout into Connect System
    And User switch to Tab "0"
    Given Reginal User able to Login into Connect system With olga_Username and Olga_Password
    And User switch to Tab "1"
